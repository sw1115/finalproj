...
sample_size <- floor(0.8*nrow(dtm_unigram))
train_ind <- sample(nrow(dtm_unigram), size = sample_size)

# use the split object to create training and testing data
unigram_training <- dtm_unigram[train_ind, ]
unigram_testing <- dtm_unigram[-train_ind, ]
```

Due to computation cost consideration, I didn't include cross-validation here.  
...
```{r randomforest_50}
rfc_50 <- randomForest(x = as.data.frame(as.matrix(unigram_training)),
                    y = token1_linkages$goal[train_ind], nTree = 50)
rfc_50
```

```{r randomforest_100}
rfc_100 <- randomForest(x = as.data.frame(as.matrix(unigram_training)),
                    y = token1_linkages$goal[train_ind], nTree = 100)
rfc_100
```
