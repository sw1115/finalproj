...
rfc_uni_50 <- randomForest(x = unigram_training[,-414],
...
rfc_uni_100 <- randomForest(x = unigram_training[,-414],
                            y = unigram_training$goal,
                            nTree = 100)
rfc_uni_100
```
...
```{r predict_rfc}
rfc_uni_50_pred <- predict(rfc_uni_50,
                           newdata = unigram_testing)

confusionMatrix(unigram_testing$goal, rfc_uni_50_pred)
```
...
Then estimate the model with at least three different values of k. Due to computational limitation we choose 80, 200 and 300 here.
```{r knn_uni}
knn_uni <- train(goal ~ .,
                 method = "knn",
...
```{r layer}
nn_uni_100 <- nnet(goal ~ .,
               data = unigram_training,
               size = 2,
               maxit = 100)
summary(nn_uni_100)

# try different maxit here to see the accuracies
nn_uni_1000 <- nnet(goal ~ .,
               data = unigram_training,
               size = 2,
               maxit = 1000)

nn_uni_final_pred <- predict(nn_uni_1000, 
                             newdata = unigram_testing,
                             type = "class")
